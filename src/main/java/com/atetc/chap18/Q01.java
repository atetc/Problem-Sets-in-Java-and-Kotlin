...
//    1. If I additional two binary numbers together, but forget to carry, the ith bit in the sum will
//    be 0 only if a and b have the same ith bit (both 0 or both 1). This is essentially an XOR.
//    2. If I additional two numbers together but only carry, I will have a 1 in the ith bit of the sum
//    only if bits i - 1 of a and b are both 1s. This is an AND, shifted.
//    3. Now, recurse until there's nothing to carry.
    
    int add(int a, int b) {
        if (b == 0) return a;
        int sum = a ^ b; // additional without carrying
        int carry = (a & b) << 1; // additional but only do carrying

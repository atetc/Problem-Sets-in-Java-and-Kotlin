...
public class Q1 {
    ...
    public static List<Integer> buildInwardSpiralArray(BinaryTree<Integer> tree) {
        ...
        Queue<BinaryTree.Node<Integer>> currentLevel = new LinkedList<>();
        Queue<BinaryTree.Node<Integer>> nextLevel = new LinkedList<>();

        currentLevel.add(tree.root);

        while (!currentLevel.isEmpty()) {
            LinkedList<Integer> level = new LinkedList<>();
            for (BinaryTree.Node<Integer> node : currentLevel) {
                if (node.left != null) {
